...
// Segmented tab bar container - hosts NTabButton children, tracks which
// tab is first/last for corner rounding (adapted from Widgets/NTabBar.qml,
    ...
    function _applyDistribution() {
        ...
        if (!distributeEvenly) {
            for (var i = 0; i < tabRow.children.length; i++)
                tabRow.children[i].Layout.fillWidth = true
            return
        }
        for (var i = 0; i < tabRow.children.length; i++) {
