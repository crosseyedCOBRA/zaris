...
// ListView wrapper with the same custom-styled scrollbar, edge-fade
// gradient masks, and smooth animated mouse-wheel scrolling as
// NScrollView.qml (adapted from Widgets/NListView.qml, MIT licensed,
// v4.7.7 - see README.md's "Third-party code" section) - forwards most of
// ListView's own properties/methods via aliases so it's a drop-in
// replacement wherever a plain ListView would otherwise go. Same three
// per-user settings toggles fixed to their more polished default as
// NScrollView.qml, for the same reason (no per-user settings system here):
// always-visible scrollbar off, smooth-scroll on, animations never
// globally disabled.
Item {
    id: root
    ...
    function positionViewAtEnd() { listView.positionViewAtEnd() }
    function forceLayout() { listView.forceLayout() }
    function cancelFlick() { listView.cancelFlick() }
    function flick(xVelocity, yVelocity) { listView.flick(xVelocity, yVelocity) }
    function incrementCurrentIndex() { listView.incrementCurrentIndex() }
    function decrementCurrentIndex() { listView.decrementCurrentIndex() }
    function indexAt(x, y) { return listView.indexAt(x, y) }
    function itemAt(x, y) { return listView.itemAt(x, y) }
