...
| Cursor theme sync (`xrdb`) | `xorg-xrdb` | `x11-xserver-utils` (bundles `xrdb` alongside `xsetroot` - same package already listed above for the wallpaper row's alternative) | `xorg-x11-server-utils` — same bundling as Debian, same package already listed above; confirmed installed and working on the Arch reference machine (`xorg-xrdb` owns `/usr/bin/xrdb` there). `start-zaris.sh` sources `apply-cursor-theme.sh`, which calls `xrdb -merge` to sync `zaris.conf`'s `cursor_theme`/`cursor_size` into the live X resource database before Zaris itself starts (see that script's own header comment for why this specific ordering matters) - without `xrdb` present, that one step is silently skipped (guarded by `command -v xrdb`), so the WM's own root cursor and any plain Xlib/XCB app would fall back to whatever cursor theme was already set some other way, while the `~/.icons/default`/GTK settings.ini syncing (which don't need `xrdb`) would still work. |
